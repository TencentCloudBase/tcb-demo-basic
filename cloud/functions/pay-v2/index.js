...
const ip = require('ip');
const {ENV, MCHID, KEY, CERT_FILE_CONTENT, TIMEOUT} = require('./config/index');
...
exports.main = async function(event) {
  ...
  switch (type) {
    ...
    case 'orderquery': {
      ...
      const {data: dbData} = await orderCollection.where({out_trade_no}).get();
    ...
    case 'payorder': {
      ...
      if (restData.trade_state === 'SUCCESS') {
        const result = await orderCollection.where({out_trade_no}).update({
          data: {
            status: 1,
