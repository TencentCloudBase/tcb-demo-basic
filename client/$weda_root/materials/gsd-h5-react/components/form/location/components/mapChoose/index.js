...
Component({
  ...
  methods: {
    ...
    regionchange(event) {
      ...
      const { circles, mapChoosedLocation } = this.data;
      ...
      if (
        mapChoosedLocation.latitude !== centerLocation.latitude &&
        mapChoosedLocation.longitude !== centerLocation.longitude
      ) {
        this.setData({
          mapChoosedLocation: marker,
        });
      }
      this.setData({
    ...
    handleConfirm() {
      const { isOutofrange, searchChoosedLocation } = this.data;
